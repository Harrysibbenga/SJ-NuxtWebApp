...
<script>
import { newsPg } from '@/services/firebase'
...
export default {
  ...
  computed: {
    post() {
      const post = {
        id: this.$store.getters['newss/getContent'].id,
        image: {
          id: this.$store.getters['newss/getContent'].imgId,
          url: this.$store.getters['newss/getContent'].url,
          alt: this.$store.getters['newss/getContent'].alt,
        },
        content: {
          type: 'news_page',
          title: this.$store.getters['newss/getContent'].title,
          content: this.$store.getters['newss/getContent'].content,
        },
      }
      return post
    },
