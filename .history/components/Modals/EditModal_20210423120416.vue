<template>
  ...
  >
    ...
    <v-card v-if="type == 'fixture'">
      <h2 class="pl-1">Insert headline image</h2>
      <PostImageUpload :image.sync="edit.image" :type="'circuit'" class="pl-3"/>
      <v-card-title>{{ post.content.date }}</v-card-title>
      <v-card-text>
        <v-card v-if="post.track" :max-width="500" tile flat>
          <v-img
            :src="post.track.url"
            :alt="post.track.alt"
            :max-width="500"
