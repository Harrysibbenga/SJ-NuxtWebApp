<template>
  <v-container>
    ...
    <v-row>
      ...
      <v-col>
        <UiEditor :content.sync="post.bio.content" />
      </v-col>
    </v-row>
    <v-row>
      <h3 class="font-weight-bold text-center pb-2">Career</h3>
      <v-col>
        <UiEditor :content.sync="post.career.content" />
      </v-col>
    </v-row>
    <UiMessage :msg="msg" />
  ...
  computed: {
    post() {
      const post = {
        ...
        career: {
          content: this.$store.getters['about/getAbout'].career.content,
        },
      }
      return post
    },
    ...
    update() {
      ...
      aboutCol
        ...
        .update({
          ...
          bio: {
            url: this.post.bio.image.url,
            alt: this.post.bio.image.alt,
            imgId: this.post.bio.image.id,
            content: this.post.bio.content,
          },
          career: {
            content: this.post.career.content,
          },
        })
        .then(() => {
          this.msg = {
