<template>
  <v-container>
    ...
    <v-row>
      <v-col>
        <h2 class="font-weight-bold text-center pb-2">Headline image</h2>
        <PostImageUpload :image.sync="post.image" :type="type" />
      </v-col>
      <v-col>
        <h2 class="font-weight-bold text-center pb-2">Car image</h2>
  ...
  computed: {
    post() {
      const post = {
        ...
        image: {
          id: this.$store.getters['car/getContent'].imgId,
          url: this.$store.getters['car/getContent'].url,
          alt: this.$store.getters['car/getContent'].alt,
        },
        content: {
          type: 'car',
