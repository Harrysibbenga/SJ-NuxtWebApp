<template>
  <v-container>
    <h2 class="font-weight-bold text-center pb-2">Shop</h2>
...
export default {
  ...
  computed: {
    post() {
      const post = {
        id: this.$store.getters['shop/getContent'].id,
        image: {
          id: this.$store.getters['shop/getContent'].imgId,
          url: this.$store.getters['shop/getContent'].url,
          alt: this.$store.getters['shop/getContent'].alt,
        },
        content: {
          type: 'shop',
          title: this.$store.getters['shop/getContent'].title,
          link: this.$store.getters['shop/getContent'].link,
        },
      }
      return post
    },
